...
class QRarea {
	...
	public function __construct($source_tab) {
		...
		foreach ($source_tab as $line) {
			...
			foreach (str_split($line) as $item) {
				...
				$arr[] = (int)$item;
				$arr_edge[] = [false, false, false, false];
		...
		$this->paths[] = [QR_AREA_TRACKER, 0, 0];
		$this->paths[] = [QR_AREA_TRACKER, 0, ($this->width - 7)];
		$this->paths[] = [QR_AREA_TRACKER, ($this->width - 7), 0];
	...
	private function addGroupElement($groupId, $h, $v, $sx, $sy) {
		...
		$this->groups[$groupId]->points[] = [$sx, $sy, false];
	}
	...
	private function detectPaths(&$group) {
		...
		$group->paths[] = [$elem[QR_AREA_X], $elem[QR_AREA_Y], $waylist];
		...
		foreach($group->points as $elem) {
			...
			if (!($edgeTab[QR_AREA_N] && $edgeTab[QR_AREA_E] && $edgeTab[QR_AREA_S] && $edgeTab[QR_AREA_W])) {
				if (!$edgeTab[QR_AREA_S]) {
					$waylistw = $this->findPath($group, $elem[QR_AREA_X], $elem[QR_AREA_Y] + 1);
					$group->paths[] = [$elem[QR_AREA_X], $elem[QR_AREA_Y] + 1, $waylistw];
				}
			}
		}
	}
